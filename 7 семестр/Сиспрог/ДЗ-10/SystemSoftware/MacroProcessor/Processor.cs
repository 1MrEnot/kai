#define FORBID_NESTED_CALLS
#define FORBID_CALL_BEFORE_DEFENITION
...
namespace SystemSoftware.MacroProcessor
{
	...
	{
		private static readonly string[] PossibleConditions = { "IF", "ELSE", "ENDIF", "AIF", "AGO",/* "WHILE", "ENDW",*/ };
		...
		private const bool UseLabelGenerator = false;

		private MacroParametersParser GetParser() => new BadMixedMacroParametersParser(this);

		private const bool LetCallMacrosBeforeDefenition = false;
		...
		public Processor(IEnumerable<string> strs)
		{
			...
			MacroParametersParser = GetParser();
		}
		...
		public void MacroSubstitution(CodeEntity se, Macro te) //подстановка
		{
			...
			foreach (var mc in PseudoMacroCalls)
			{
				...
				if (mc.Operation == fixedMacroName)
				{
					...
					{
						if (AssemblerCode[i].Operation == mc.Operation &&
						    AssemblerCode[i].IsRemove &&
						    mc.CallNumber == AssemblerCode[i].CallNumber)
						{
		...
		public void FirstRunStep(CodeEntity se, Macro te)
		{
			...
			else if (operation == "MACRO")
			{
				...
				if (!macroNameRegex.IsMatch(macroName)) 
					throw new CustomException($"Некорректное имя макроса {macroName}. Содержит недопустимые символы и/или имеет некорректный формат.");
				//if (operands.Count != 0)
				//{
...
#if FORBID_CALL_BEFORE_DEFENITION
					// Ругаемся на не опережающее определение макроса
					if (AssemblerCode.Any(c => c.Operation == label))
						throw new CustomException($"Не опережающее описание макроса {label}");
#endif
				}
     ...
     //                       }
			...
			else
			{
				...
				{
					if (te == MacrosStorage.Root && operation.In(PossibleConditions))
					{
					...
					else
					{
						...
						{
							// TODO: OPTIONAL LABEL GENERATOR
							var newLabel = $"$_{se.Label}_{te.Name}_{te.UniqueLableForLoop}";
							se.Label = UseLabelGenerator ? newLabel : string.Empty;							
   ...
   //         }
			...
			{
				if (sourceLine.Operation == "MACRO") 
					macroCount++;
				if (sourceLine.Operation == "MEND") 
					macroCount--;
				if (macroCount != 0) 
					continue;
	...
	public static class CheckBody
	{
		...
		{
#if FORBID_NESTED_CALLS
			if (parent.Name != null)
				throw new CustomException($"Макровызовы внутри макроса запрещены: {parent.Name}");
#endif

			var current = parent;
			var list = new List<Macro>();
			while (current.PreviousMacros?.Name != null)
			{
