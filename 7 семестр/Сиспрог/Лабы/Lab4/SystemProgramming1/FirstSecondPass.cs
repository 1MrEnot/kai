   ...
   class FirstSecondPass
    {
       ...
       public bool  MemoryCheck()
       {
           ...
           {
               ErrorMessage = "Исходный текст программы --> Строка {1} Выход за границы доступной памяти";
               return false;
           }
           return true;
       }
       ...
       public bool Check_operation_code_table(ref string[,] OperationCodeTable) //проверяем таблицу  операций
        {
           ...
           for (int i = 0; i < rows; i++)
           {
               ...
               {
                   ErrorMessage = "Таблица кодов операций --> Строка {" + (i + 1) + "} Ошибка! Нет команды такого размера:" + OperationCodeTable[i, 2] + " байт";
                   return false;
               }

                //changed code
                if (Check.OnlyNumbers(OperationCodeTable[i, 0]))
                {
               ...
               if (Check.IsAdressPossible(OperationCodeTable[i, 1]))
               {
                   ...
                   {
                       ErrorMessage = "Таблица кодов операций --> Строка {" + (i + 1) + "} Ошибка! Поле код команды является зарезервированным словом";
                       return false;
                   }
                   ...
                   if (count > 63)
                   {
                       ErrorMessage = "Таблица кодов операций --> Строка {" + (i + 1) + "} Ошибка! Поле код команды не должен превышать 3F";
                       return false;
                   }
                   else
                   {
       ...
       public int ReplaceLabel(string label,string label_adr,string currentAdr)
       {
           ...
           List<List<string>> temp_labels = new List<List<string>>();
                       ...
                       if (lable_table[2][i] == exit_table[1][j])// добавится имя секции
                       {
                           ...
                           if (exit_table[3][j].Length > 0)
                           {
                               ...
                               if (indexOfSubstring > -1)
                               {
                                   switch (Type_of_adr)
                                   {
                                       case 1: exit_table[3][j] = exit_table[3][j].Replace("#" + label + "#", label_adr); break;
                                       // case 2: error_message = error_message + "Относительня адресация недопустима"; break;
                                       //exit_table[3][j] = exit_table[3][j].Replace("*" + label + "*", Converting.SubHex(n2, currentAdr)); break;
                                   }
           ...
           //удаляем все записи из ТСИ, которые добавили в отдельный список 
           //for (int i = 0; i < labels.Count(); i++)
           //{
           //    for (int j = 0; j < lable_table[0].Count; j++)
           //    {
           //        if (lable_table[0][j] == labels[0][i] && lable_table[1][j] == labels[1][i] && lable_table[2][j] == labels[2][i])
           //        {
           //            lable_table[0].RemoveAt(j);
           //            lable_table[1].RemoveAt(j);
           //            lable_table[2].RemoveAt(j);
           //        }
           //    }
           //}
       ...
       public bool Pass(DataGridView dataGrid_symbol_table, DataGridView dataGrid_exit_table,int row)
       {
            ...
            if (StartFlag == 1)
            {
               ...
               {
                   ErrorMessage = "Исходный текст программы --> Синтаксическая ошибка строка = " + (row + 1);
                   return false;
               }
               ...
               /////////////////////////////////////////   ПОЛЕ МЕТКИ  //////////////////////////////////////////////////////
                ...
                for (int k = 0; k <= operation_code_table.GetUpperBound(0)-1; k++)
                {
                    ...
                    {
                        ErrorMessage = "Таблица кодов операций --> Строка {" + (k + 1) + "} Ошибка! Символическое имя не может совпадать с названием команды";
                        return false;
                    }
                }
                ...
                foreach (string str in EXT_NAMES)
                {
                    ...
                    {
                        ErrorMessage = "Исходный текст программы --> Ошибка! Символическое имя не может совпадать с названием программы";
                        return false;
                    }
                }
                ...
                if (label.Length > 0 && MKOP.ToUpper() != "START")
                {
                    ...
                    else
                    {
                        ...
                        if (name_address == "")
                        {
                            ...
                            //ReplaceLabel(label, Converting.ToSixChars(Converting.DecToHex(addressCounter)));
                        }
                        //если СИ найдено и ему назначен адрес, то ошибка
                        else
                        {
                            ErrorMessage = "Исходный текст программы --> Ошибка! Повторение символьных имен недопустимо";
                            return false;
                        }
                    }
                }
                        ...
                        case "START":
                            {
                                ...
                                if (row == 0 && StartFlag == 0)
                                {
                                    ...
                                    if (Check.IsAdressPossible(Operand1) || Operand1 == "")
                                    {
                                        ...
                                        if (addressCounter == 0)
                                        {
                                            ErrorMessage = "Исходный текст программы --> Строка {1} Ошибка! Адрес начала программы не должен быть = 0";
                                            return false;
                                        }
                                        //адрес начала программы не может превышать объем памяти
                                        if (addressCounter > maxMemmoryAdr)
                                        {
                                            ErrorMessage = "Исходный текст программы --> Строка {1} Ошибка! Адрес программы выходит за диапазон памяти";
                                            return false;
                                        }

                                        if (label == "")
                                        {
                                            ErrorMessage = "Исходный текст программы --> Строка {1} Ошибка! Не задано имя программы";
                                            return false;
                                        }
                                        if (label.Length > 10)
                                        {
                                            ErrorMessage = "Исходный текст программы --> Строка {1} Ошибка! Превышена длина имени программы(>10 символов)";
                                            return false;
                                        }
                                        ...
                                        for (int k = 0; k <= operation_code_table.GetUpperBound(0); k++)
                                        {
                                            if (Equals(label.ToUpper(), operation_code_table[k, 0].ToUpper()))
                                            {
                                                ErrorMessage = "Исходный текст программы --> Строка {" + (k + 1) + "} Ошибка! Имя программы не может совпадать с названием команды";
                                                return false;
                                            }
                                        }
                                        ...
                                        if (Operand2.Length > 0)
                                            ErrorMessage = ErrorMessage + "Исходный текст программы --> Строка {1} Ошибка! Второй операнд директивы START не рассматривается\n";

                                    }
                                    else
                                    {
                                        ErrorMessage = "Исходный текст программы --> Строка {1} Ошибка! Неверный адрес начала программы";
                                        return false;
                                    }
                                ...
                                {
                                    ErrorMessage = "Исходный текст программы --> Строка {1} Ошибка в директиве START";
                                    return false;
                                }
                        ...
                        case "WORD":
                            {
                                if (StartFlag == 1)
                                {
                                    ...
                                    if (int.TryParse(Operand1, out number))
                                    {
                                        if (number >= 0 && number <= maxMemmoryAdr)
                                        {
                                            ...
                                            if (Operand2.Length > 0)
                                                ErrorMessage = ErrorMessage + "Исходный текст программы --> Ошибка! Второй операнд директивы WORD не рассматривается\n";
                                        }
                                        else
                                        {
                                            ErrorMessage = "Исходный текст программы -->  Ошибка! Отрицательное число, либо превышено максимальное значение числа";
                                            return false;
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if (Operand1.Length == 1 && Operand1 == "?")
                                        {
                                            ...
                                            if (Operand2.Length > 0)
                                                ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд директивы WORD не рассматривается\n";
                                        }
                                        else
                                        {
                                            ErrorMessage = "Исходный текст программы --> Невозможно выполнить преобразование в число " + Operand1;
                                            return false;
                                        }
                                    }
                                }
                                ...
                                {
                                    ErrorMessage = "Исходный текст программы --> Не найдена директива START";
                                    return false;
                                }
                            }
                            break;
                        case "BYTE":
                            {
                                if (StartFlag == 1)
                                {
                                    ...
                                    if (int.TryParse(Operand1, out number))
                                    {
                                        if (number >= 0 && number <= 255)
                                        {
                                            ...
                                            if (Operand2.Length > 0)
                                                ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд директивы BYTE не рассматривается\n";
                                        }

                                        else
                                        {
                                            ErrorMessage = "Исходный текст программы --> Отрицательное число, либо превышеноо максимальное значение числа";
                                            return false;
                                        }
                                    }
                                    ...
                                    else
                                    {
                                        ...
                                        if (symbols != "")
                                        {
                                            ...
                                            {
                                                ErrorMessage = "Превышена длина строки";
                                                return false;
                                            }
                                            ...
                                            if (error == 1)
                                            {
                                                ErrorMessage = ErrorMessage + "Исходный текст программы --> Ошибка в операнде,код отсутствует в ТСИ " + (row + 1) + " строка";
                                                return false;
                                            }
                                            ...
                                            if (Operand2.Length > 0)
                                                ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд директивы BYTE не рассматривается" + "//r//n";
                                            break;
                                        }
                                        ...
                                        if (symbols != "")
                                        {
                                            ...
                                            if ((lenght % 2) == 0)
                                            {
                                                ...
                                                if (Operand2.Length > 0)
                                                    ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд директивы BYTE не рассматривается" + '\r' + '\n';

                                                break;
                                            }
                                            else
                                            {
                                                ErrorMessage = "Исходный текст программы --> Невозможно преобразовать BYTE нечетное количество символов";
                                                return false;
                                            }
                                        }
                                        ...
                                        {
                                            ErrorMessage = "Исходный текст программы --> Ошибка! Неверный формат строки " + Operand1;
                                            return false;
                                        }
                                    }
                                }
                                ...
                                {
                                    ErrorMessage = "Исходный текст программы --> Ошибка! Не найдена директива START";
                                    return false;
                                }
                                break;
                            }
                                    ...
                                    if (int.TryParse(Operand1, out number))
                                    {
                                        if (number > 0)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (Operand2.Length > 0)
                                                    ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд директивы RESB не рассматривается \n";
                                            }

                                        }
                                        else
                                        {
                                            ErrorMessage = "Исходный текст программы --> Ошибка! Количество байт равно нулю или меньше нуля";
                                            return false;
                                        }
                                    }
                                    else
                                    {
                                        ErrorMessage = "Исходный текст программы --> Ошибка! Невозможно выполнить преобразование в число " + Operand1;
                                        return false;
                                    }
                                }
                                else
                                ...
                                if (StartFlag == 1)
                                {
                                    ...
                                    if (int.TryParse(Operand1, out number))
                                    {
                                        if (number > 0)
                                        {
                                            ...
                                            if (addressCounter < 0 || addressCounter > maxMemmoryAdr)
                                            {
                                                ErrorMessage = "Исходный текст программы --> Ошибка! Выход за границы доступной памяти";
                                                return false;
                                            }


                                            if (Operand2.Length > 0)
                                                ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд директивы RESW не рассматривается \n";


                                        }
                                        else
                                        {
                                            ErrorMessage = "Исходный текст программы --> Количество слов равно нулю или меньше нуля";
                                            return false;
                                        }
                                    }
                                    else
                        ...
                        case "END":
                            {
                                if (StartFlag == 1 && EndFlag == 0)
                                {
                                    ...
                                    if (Operand1.Length == 0)
                                    {
                                        ...
                                        if (Operand2.Length > 0)
                                            ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд директивы END не рассматривается" + '\r' + '\n';
                                    ...
                                    {
                                        ErrorMessage = "Исходный текст программы --> Ошибка! Найдено неопределенное внешнее имя " + lable_table[0][num];
                                        return false;
                                    }
                                ...
                                //break;
                                    ...
                                    else
                                    {
                                        if (Check.IsAdressPossible(Operand1))
                                        {
                                            ...
                                            {
                                                ErrorMessage = "Исходный текст программы --> Ошибка! Адрес точки выхода неверен";
                                                return false;
                                            }
                                        ...
                                        {
                                            ErrorMessage = "Исходный текст программы --> Ошибка! Неверный адрес входа в программу";
                                            return false;
                                        }
                                    }
                                ...
                                {
                                    ErrorMessage = "Исходный текст программы --> Ошибка в директиве END";
                                    return false;
                                }
                            }
                            break;
                ...
                else
                {
                    if (StartFlag == 1)
                    {
                        ...
                        if (MKOP.Length > 0)
                        {
                            ...
                            if (num > -1)
                            {
                                ...
                                if (operation_code_table[num, 2] == "1")
                                {
                                    ...
                                    if (Operand1.Length > 0 || Operand2.Length > 0)
                                        ErrorMessage = ErrorMessage + "Исходный текст программы --> Ошибка! Операнды не рассматриваются в команде " + operation_code_table[num, 0] + "\r\n";
                                }
                                //            else
                                //ДЛИНА КОМАНДЫ = 2
                                ...
                                if (operation_code_table[num, 2] == "2")
                                {
                                    ...
                                    if (int.TryParse(Operand1, out number))
                                    {
                                        if (number >= 0 && number <= 255)
                                        {
                                            ...
                                            if (Operand2.Length > 0)
                                                ErrorMessage = ErrorMessage + "Исходный текст программы --> Второй операнд команды" + operation_code_table[num, 0] + " не рассматривается \n";
                                        }
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            ErrorMessage = "Исходный текст программы --> Ошибка в команде " + operation_code_table[num, 0];
                                            return false;
                                        }
                                    ...
                                    if (operation_code_table[num, 2] == "4")
                                    {
                                        ...
                                        if (Operand1.Length > 0)
                                        {
                                                ...
                                                {
                                                    ErrorMessage = ErrorMessage + "Исходный текст программы --> Относительная адресация недопустима"; return false;
                                                }

                                                //Так как в абсолютном ассемблере нет относительной адресации , то выключим её
                                                ...
                                                //    else { error_message = error_message + "Ошибка в символическом имени"; return false; }
                                                //else return false;
                                                ...
                                                if (Check.PossibleLabelName(Operand1) && Check.OnlySymbols(Convert.ToString(Operand1[0]))) { }
                                                else { ErrorMessage = ErrorMessage + "Исходный текст программы --> Ошибка в символическом имени"; return false; }
                                        ...
                                        {
                                            ErrorMessage = "Исходный текст программы --> Ошибка! Не найден операнд в строке:" + (row + 1);
                                            return false;
                                        }
                                        ...
                                        for (int k = 0; k <= operation_code_table.GetUpperBound(0) - 1; k++)
                                        {
                                            ...
                                            {
                                                ErrorMessage = "Исходный текст программы --> Строка {" + (k + 1) + "} Ошибка! Символическое имя не может совпадать с названием команды";
                                                return false;
                                            }
                                        }

                                        //foreach (var str in EXT_NAMES)
                                        //{
                                        ...
                                        //chanded code
                                        foreach (var str in EXT_NAMES.Select((value, index) => new { value, index }))
                                        {
                                            if (Equals(Operand1.ToUpper(), str.value.ToUpper()))
                                            {
                                                ErrorMessage = "Исходный текст программы --> Строка {" + (str.index + 1) + "} Ошибка! Символическое имя не может совпадать с названием программы";
                                                return false;
                                            }
                                        }
                                    ...
                                    if (Check.IsDirective(Operand1) || Check.OnlyRegisters(Operand1))
                                        {
                                            ErrorMessage = "Исходный текст программы --> Ошибка! Символическое имя не может совпадать с системным именем";
                                            return false;
                                        }
                                        ...
                                        if (finded_row > -1)
                                        {
                                            ...
                                            {
                                                lable_table[0].Insert(finded_row + 1, Operand1.ToUpper());//----
                                                lable_table[1].Insert(finded_row+1, "");//----
                                                lable_table[2].Insert(finded_row+1, Converting.ToSixChars(Converting.DecToHex(addressCounter)));//---
                                    ...
                                    {
                                        ErrorMessage = "Исходный текст программы --> Ошибка! Размер команды больше установленного";
                                        return false;
                                    }
                            ...
                            {
                                ErrorMessage = "Исходный текст программы --> Ошибка! МКОП отсутствует в ТКО";
                                return false;
                            }
                        }
                        else
                        {
                            ErrorMessage = "Исходный текст программы --> Ошибка! Не найден МКОП";
                            return false;
                        }
                    }
           ...
           //Помещаем сформированную Таблицу вспомогательных имен в датагрид
               for (int j = 0; j < lable_table[1].Count; j++)
               {
                  ...
                  // dataGrid_symbol_table.Rows[j].Cells[3].Value = lable_table[3][j];
               }
