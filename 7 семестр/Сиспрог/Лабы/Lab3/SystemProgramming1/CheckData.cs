...
namespace SystemProgramming1
{
    class CheckData
    {
        ...
        {
            string[] system_directives = { "START", "END", "BYTE", "WORD", "RESB", "RESW", "CSECT", "EXTREF", "EXTDEF" };
        ...
        public static string ByteString(string Operand1)
        {
            if ((Operand1.Length > 3) && (Operand1[0] == 'X') && (Operand1[1] == '"') && (Operand1[Operand1.Length - 1] == '"'))
            {
                ...
                if (!CheckData.IsAdressPossible(text))
                    return "";
                return text;
            }
            return "";
        ...
        public static bool ExternalNames(List<List<string>> lable_table)
        {
            for (int i = 0; i < lable_table.Count; i++)
            {

                if (lable_table[1][i].ToString() == "" && lable_table[3][i].ToString() == "ВИ")
                    return false;
            }
            return true;
        }
