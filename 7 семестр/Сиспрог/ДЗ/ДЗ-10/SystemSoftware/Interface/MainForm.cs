	...
	public partial class MainForm : Form
	{
		...
		private void OnNextStep(object sender, EventArgs e)
		{
			...
			try
			{
				...
				else if (_program.SourceCodeIndex == 0)
                {
		...
		private void OnFirstRun(object sender, EventArgs e)
		{
			...
			while (true) //демаем по одному шагу пока не произойдет ошибка или пока не закончится текст программы
			{
		...
		private void OnRefreshAll(object sender, EventArgs e)
		{
			...
			_program.PrintVariablesTable(dgvVariables);
			_program.PrintMacroNameTable(dgvMacroNames);
			//_program.PrintMacroprocessorDirectives(tbAssemblerDirectives, tbMacrogenerationDirectives);
		...
		private void OnSaveIntoFile(object sender, EventArgs e)
		{
			...
			if (sfd.ShowDialog() == DialogResult.OK)
			{
				...
				var temp = tbAssemblerCode.Text.Split('\n').ToList();
		...
		private void dgvMacroNames_CellContentClick(object sender, DataGridViewCellEventArgs e)
		{
		}
	}
}
