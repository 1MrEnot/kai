#define FORBID_NESTED_DEFINITIONS
	...
	public class Macro
	{
		...
		public Processor CallMacro(List<CodeEntity> source)
		{
			...
			{				
				...
				{
#if FORBID_NESTED_DEFINITIONS
					if (current.MacroParentName != null)
						throw new CustomException($"Обнаружено вложенное макроопределение: {current.SourceString}");
#endif
					macroCount++;
				}
