...
namespace SystemSoftware.MacroProcessor
{
	...
	public class PositionMacroParametersParser : MacroParametersParser
	{
		...
		public override List<MacroParameter> Parse(List<string> operands, string macroName)
		{
			...
			foreach (var currentOperand in operands)
			{
	...
	public class MixedMacroParametersParser : MacroParametersParser
	{
		...
		public override List<MacroParameter> Parse(List<string> operands, string macroName)
		{
			...
			for (var i = 0; i < lastPositionParameterIdx; i++)
			{
				var currentOperand = operands.ToArray()[i];

				Helpers.CheckNames(currentOperand, false, true);
			...
			{
				for (var i = firstKeyParameterIdx; i < operands.Count(); i++)
				{
					var currentOperand = operands.ToArray()[i];

					var paramName = currentOperand.Split('=')[0];

					Helpers.CheckNames(paramName, isParameter: true);
					...
					var parameterName = vals[0];
					var defaultValue = vals.Length == 2 
						? vals[1] 
						: currentOperand.Substring(currentOperand.IndexOf('=') + 1);
	...
	/// Утилита для парсинга смешанных параметров.
	/// </summary>
	public class BadMixedMacroParametersParser : MacroParametersParser
	{
		public BadMixedMacroParametersParser(Processor processor)
			: base(processor)
		{
		...
		public override List<MacroParameter> Parse(List<string> operands, string macroName)
		{
			...
			if (!operands.All(o => o.Contains("=")))
				throw new CustomException($"В макросе {macroName} указан позиционный параметр");
