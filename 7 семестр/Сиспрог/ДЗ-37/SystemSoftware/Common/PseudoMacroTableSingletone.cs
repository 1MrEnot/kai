...
namespace SystemSoftware.Common
{
	...
	public class PseudoMacroTableSingletone
	{
		...
		{
			foreach (var se in Instance)
			{
				var currentMacro = se.MacroParentName == null ? null : MacrosStorage.Entities.FirstOrDefault(m => m.Name == se.MacroParentName);
				if (MacrosStorage.FindVisibleMacro(currentMacro,se.Operation)==null &&
				...
				{
					throw new CustomException("Неизвестное определение \"" + se.Operation + "\"");
				}
			}
		}
	}
